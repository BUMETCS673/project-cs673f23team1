...
import com.aceteam.tm.user.persistence.mapper.DynamicPoMapper;
    ...
    public PageInfo<DynamicDTO> getByUserId(Long userId, Integer currentPage, Integer pageSize) {
        ...
        if (CollectionUtils.isNotEmpty(pageInfo.getList())) {
            pageInfo.getList().forEach(dynamicDTO -> {
                ...
                // 文章相关
                if (DynamicTypeEnum.writePost.name().equals(dynamicDTO.getType()) ||
                        DynamicTypeEnum.likePost.name().equals(dynamicDTO.getType()) ||
                        DynamicTypeEnum.commentPost.name().equals(dynamicDTO.getType())) {
                    List<PostDTO> postDTOS = postService.getByIds(Collections.singletonList(Integer.parseInt(dynamicDTO.getObjectId())), null, null);
                    dynamicDTO.setTitle(postDTOS.get(0).getTitle());
                }
                // 评论相关
                if (DynamicTypeEnum.likeComment.name().equals(dynamicDTO.getType()) ||
                        DynamicTypeEnum.commentReply.name().equals(dynamicDTO.getType())) {
                    List<PostDTO> postDTOS = postService.getByIds(Collections.singletonList(Integer.parseInt(dynamicDTO.getObjectId())), null, null);
                    dynamicDTO.setTitle(postDTOS.get(0).getTitle() + " > " + CommonUtils.html2Text(commentService.getById(dynamicDTO.getCommentId()).getContent()));
                }
...
//        LocalDateTime startTime = LocalDateTime.parse("2000-01-01 00:00:00", df);
        ...
        // 写文章
        List<PostDTO> postDTOS = postService.getPassAll(startTime, endTime);
        if (CollectionUtils.isNotEmpty(postDTOS)) {
            postDTOS.forEach(postDTO -> {
                DynamicDTO dynamicDTO = new DynamicDTO();
                dynamicDTO.setType(DynamicTypeEnum.writePost.name());
                dynamicDTO.setUserId(postDTO.getCreateUser());
                dynamicDTO.setObjectId(String.valueOf(postDTO.getId()));
                dynamicDTO.setCreateTime(postDTO.getCreateTime());
        ...
        if (CollectionUtils.isNotEmpty(likeDTOS)) {
            likeDTOS.forEach(likeDTO -> {
                DynamicDTO dynamicDTO = new DynamicDTO();
                dynamicDTO.setType(DynamicTypeEnum.likePost.name());
                dynamicDTO.setUserId(likeDTO.getLikeUser());
                dynamicDTO.setObjectId(String.valueOf(likeDTO.getPostId()));
        ...
        if (CollectionUtils.isNotEmpty(likeCommentDTOS)) {
            likeCommentDTOS.forEach(likeCommentDTO -> {
                ...
                dynamicDTO.setObjectId(String.valueOf(commentService.getPostIdByCommentId(likeCommentDTO.getCommentId())));
        ...
        // 评论文章
        List<CommentDTO> commentDTOS = commentService.getAllPostComment(startTime, endTime);
        if (CollectionUtils.isNotEmpty(commentDTOS)) {
            commentDTOS.forEach(commentDTO -> {
                DynamicDTO dynamicDTO = new DynamicDTO();
                dynamicDTO.setType(DynamicTypeEnum.commentPost.name());
                dynamicDTO.setUserId(commentDTO.getCommentUser());
                dynamicDTO.setObjectId(String.valueOf(commentDTO.getPostId()));
